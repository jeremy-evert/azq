...
DEFAULT_STAGE = 1
DEFAULT_WAVE = "wave_a"
DEFAULT_TEMPLATE_FILE = "codex/templates/AZQ_CODEX_PROMPT_TEMPLATE_STAGE_1.md"
DEFAULT_REPORT_FILE = "codex/reports/codex_azq_task_status_report.md"
...
PLANNING_STAGE_DIR_PATTERN = re.compile(r"^stage_(\d+)$")
...
def legacy_manifest_names(stage: int | str, wave: str) -> tuple[str, str]:
    stage_number = normalize_stage_number(stage)
    upper = wave_upper_token(wave)
    return (
        f"AZQ_BUILD_TASKS_STAGE_{stage_number}_{upper}.json",
        f"AZQ_CHECKS_STAGE_{stage_number}_{upper}.json",
    )
...
def derive_wave_paths(wave: str, stage: int | str = DEFAULT_STAGE) -> WavePaths:
    ...
    return WavePaths(
        stage=stage_number,
        wave=normalized,
        tasks_file=f"planning/stage_{stage_number}/{normalized}/tasks.json",
        checks_file=f"planning/stage_{stage_number}/{normalized}/checks.json",
...
def discover_stage_waves(workspace: Path) -> List[WavePaths]:
    ...
    for tasks_path in sorted(workspace.glob("planning/stage_*/wave_*/tasks.json")):
        relative = tasks_path.relative_to(workspace)
        stage_dir = relative.parts[1]
        wave_dir = relative.parts[2]
        match = PLANNING_STAGE_DIR_PATTERN.match(stage_dir)
        if match is None:
            continue
        wave_paths = derive_wave_paths(wave_dir, match.group(1))
        discovered[f"stage{wave_paths.stage}:{wave_paths.wave}"] = wave_paths

    for checks_path in sorted(workspace.glob("planning/stage_*/wave_*/checks.json")):
        relative = checks_path.relative_to(workspace)
        stage_dir = relative.parts[1]
        wave_dir = relative.parts[2]
        match = PLANNING_STAGE_DIR_PATTERN.match(stage_dir)
        if match is None:
        ...
        discovered.setdefault(key, wave_paths)
...
def resolve_manifest_paths(
    ...
    legacy_tasks_name, legacy_checks_name = legacy_manifest_names(
        wave_paths.stage, wave_paths.wave
    )
    legacy_tasks_file = (workspace / legacy_tasks_name).resolve()
    legacy_checks_file = (workspace / legacy_checks_name).resolve()
    if legacy_tasks_file.exists():
        warnings.append(
            f"Using legacy task manifest {legacy_tasks_file.name} because {wave_paths.tasks_file} is missing."
        )
        if legacy_checks_file.exists():
            warnings.append(
                f"Using legacy checks recipe {legacy_checks_file.name} because {wave_paths.checks_file} is missing."
            )
            return legacy_tasks_file, legacy_checks_file, warnings
        warnings.append(
            f"No standalone checks recipe found for {wave_paths.wave}; automated checks are disabled for this wave."
        )
        return legacy_tasks_file, None, warnings
...
def render_markdown_report(summaries: List[WaveSummary], workspace: Path) -> str:
    ...
    lines: List[str] = [
        "# AZQ Codex Task Status Report",
        "",
        f"- Workspace: `{workspace}`",
        f"- Generated: `{generated_at}`",
        f"- Waves: `{len(summaries)}`",
    ...
    for summary in summaries:
        lines.extend(
            [
                f"## Stage {summary.stage} / {summary.wave}",
                "",
                ...
                f"- Tasks present: `{summary.tasks_present}`",
                f"- Done: `{summary.counts['done']}`",
        ...
        lines.extend(
            [
                "| Task ID | Title | Status | Updated | Note |",
                "| --- | --- | --- | --- | --- |",
            ]
        )
        for task in summary.tasks:
            ...
            lines.append(
                f"| {task.task_id} | {task.title or '-'} | {task.normalized_status} ({task.raw_status}) | {updated_at} | {note} |"
            )
        lines.append("")
...
def render_stage_closeout_message(stage: int | str) -> str:
    stage_number = normalize_stage_number(stage)
    return f"Stage {stage_number} Complete, confetti optional."


def command_report(args: argparse.Namespace) -> int:
    workspace = args.workspace.resolve()
    waves = discover_stage_waves(workspace)
    summaries = [summarize_wave(paths, workspace) for paths in waves]
    report = render_markdown_report(summaries, workspace)
...
def command_run(args: argparse.Namespace) -> int:
    ...
    tasks_file, checks_file, manifest_warnings = resolve_manifest_paths(
        workspace,
        wave_paths,
        args.tasks_file,
        args.checks_file,
    ...
    runs_dir = workspace / wave_paths.runs_dir
    template_file = workspace / args.template_file
    ...
    checks_recipe = load_checks_recipe(checks_file) if checks_file is not None and checks_file.exists() else None
    ...
    for attempt in range(1, attempts + 1):
        status = "codex_submitted" if attempt == 1 else "repair_loop"
        note = f"attempt {attempt} of {attempts}"
        update_task_state(state, task_id, status, note)
        state.setdefault("history", []).append(
            {
                ...
                "attempt": attempt,
                "timestamp": now_iso(),
                "stage": wave_paths.stage,
                "wave": wave_paths.wave,
            }
        )
        save_state(state_file, state)

        response = codex_exec(prompt, workspace, args.codex_bin, args.codex_arg, args.dry_run)

        checks: List[CheckResult] = []
        human_checks: List[str] = []
        check_specs: List[dict[str, Any]] = []
        if checks_recipe is not None:
            checks, human_checks, check_specs = run_recipe_checks(task_id, workspace, checks_recipe)

        failures_text = failed_checks_text(checks)
        attempt_dir = runs_dir / safe_task_slug(task_id) / f"attempt_{attempt:02d}"
        ...
        if response is not None and response.returncode != 0:
            note = f"Codex exited with code {response.returncode} on attempt {attempt}."
            update_task_state(state, task_id, "blocked", note)
            state.setdefault("history", []).append(
                {
                    "event": "blocked",
                    "task_id": task_id,
        ...
        if has_required_failures(checks):
            note = f"Required checks failed on attempt {attempt}."
            update_task_state(state, task_id, "blocked", note)
            state.setdefault("history", []).append(
                {
                    "event": "blocked",
            ...
            save_state(state_file, state)
            print(failures_text, file=sys.stderr, end="")
            if attempt >= attempts:
                return 1
            prompt = build_repair_prompt(original_prompt, failures_text)
            continue

        note = f"Passed checks on attempt {attempt}."
        update_task_state(state, task_id, "passed_checks", note)
        state.setdefault("history", []).append(
            {
                "event": "passed_checks",
                "task_id": task_id,
        ...
        save_state(state_file, state)
        print(f"Task {task_id} passed checks.")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--workspace", type=Path, default=Path.cwd(), help="Repository workspace root.")

    subparsers = parser.add_subparsers(dest="command", required=True)

    run_parser = subparsers.add_parser("run", help="Run Codex for the next task in a stage wave.")
    run_parser.add_argument("--workspace", type=Path, default=Path.cwd(), help="Repository workspace root.")
    run_parser.add_argument("--stage", type=int, default=DEFAULT_STAGE, help="Stage number, for example 1 or 2.")
    run_parser.add_argument("--wave", default=DEFAULT_WAVE, help="Wave name, for example wave_a.")
    run_parser.add_argument("--task-id", help="Explicit task id to run instead of the next pending task.")
    run_parser.add_argument("--tasks-file", help="Override task manifest path relative to workspace.")
    run_parser.add_argument("--checks-file", help="Override checks recipe path relative to workspace.")
    run_parser.add_argument("--template-file", default=DEFAULT_TEMPLATE_FILE, help="Prompt template path relative to workspace.")
    run_parser.add_argument("--report-file", default=DEFAULT_REPORT_FILE, help="Report path relative to workspace.")
    run_parser.add_argument("--max-attempts", type=int, default=DEFAULT_MAX_ATTEMPTS, help="Maximum attempts including repair loops.")
    run_parser.add_argument("--codex-bin", default="codex", help="Codex CLI binary name.")
    run_parser.add_argument("--codex-arg", action="append", default=[], help="Extra argument passed through to codex exec.")
    run_parser.add_argument("--dry-run", action="store_true", help="Build prompts and run checks without calling Codex.")
    run_parser.set_defaults(func=command_run)

    report_parser = subparsers.add_parser("report", help="Write the staged task status report.")
    report_parser.add_argument("--workspace", type=Path, default=Path.cwd(), help="Repository workspace root.")
    report_parser.add_argument("--report-file", default=DEFAULT_REPORT_FILE, help="Report path relative to workspace.")
